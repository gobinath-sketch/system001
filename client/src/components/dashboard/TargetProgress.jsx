        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
            <div className="flex justify-between items-center mb-4">
                ...
                <select
                    ...
                    className="text-sm border-gray-300 border rounded-md px-2 py-1 focus:outline-none focus:border-brand-blue"
                >
            ...
            <div className="w-full bg-gray-200 rounded-full h-4 mb-4 overflow-hidden relative">
                <div
                    className={`${getColor(data.percentage)} h-4 rounded-full transition-all duration-1000`}
                    style={{ width: `${Math.min(data.percentage, 100)}%` }}
                ></div>
                {data.percentage > 100 && (
                    <div className="absolute top-0 right-0 h-4 w-1 bg-white animate-pulse" title="Overachieved!"></div>
                )}
            </div>
