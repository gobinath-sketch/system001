...
const EscalationWidget = ({ opportunities, onEscalate }) => {
    ...
    const handlePush = async (opp) => {
        const gp = calculateGP(opp);
        const tov = opp.commonDetails?.tov || 0;
        const totalExpense = opp.financials?.totalExpense || 0;
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-red-100">
            ...
            <div className="space-y-4">
                {lowGPOpps.map(opp => {
                    const gp = calculateGP(opp);
                    return (
                        <div key={opp._id} className="border border-red-100 rounded-lg p-3 bg-red-50 flex justify-between items-center">
                            <div>
