            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {opportunities.length > 0 ? opportunities.map(opp => <tr key={opp._id} className="hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/opportunities/${opp._id}?tab=documents`)}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                    {opp.proposalDocument ? <CheckCircle size={18} className="inline text-green-500" /> : <AlertCircle size={18} className="inline text-yellow-500" />}
                                </td>

                                {/* PO Status */}
                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                    {opp.poDocument ? <CheckCircle size={18} className="inline text-green-500" /> : <AlertCircle size={18} className="inline text-yellow-500" />}
                                </td>

                                {/* Invoice Status */}
                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                    {opp.invoiceDocument ? <CheckCircle size={18} className="inline text-green-500" /> : <span className="text-gray-300">-</span>}
                                </td>
                            </tr>) : <tr>
