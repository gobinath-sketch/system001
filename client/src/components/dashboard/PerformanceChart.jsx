...
const PerformanceChart = ({
  ...
  return <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
            ...
            <div className="space-y-6">
                {teamData.map((member, idx) => <div key={idx}>
                        ...
                        <div className="h-6 w-full bg-gray-100 rounded-lg relative overflow-hidden flex">
                            {/* Achieved Bar */}
                            <div className="h-full bg-blue-500 absolute top-0 left-0 z-10 rounded-l-lg transition-all" style={{
            width: `${member.achieved / maxVal * 100}%`
          }} title={`Achieved: $${member.achieved}`}></div>
                             ...
                             */}
                            <div className="h-full bg-blue-200 absolute top-0 left-0 rounded-lg" style={{
            width: `${member.target / maxVal * 100}%`
          }} title={`Target: $${member.target}`}></div>
                        </div>
                        {member.achieved > member.target && <div className="text-xs text-green-600 font-bold mt-1 text-right">
