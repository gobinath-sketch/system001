...
const AddSMEModal = ({ isOpen, onClose, onSuccess, smeToEdit = null }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.contactNumber) {
            const { valid, message } = validateMobile(formData.contactNumber);
            if (!valid) return addToast(`SME Contact: ${message}`, 'error');
        }

        if (formData.smeType === 'Company') {
            const { valid, message } = validateMobile(formData.companyContactNumber);
        ...
        if (formData.bankDetails.accountNumber) {
            const { valid, message } = validateBankAccount(formData.bankDetails.accountNumber);
            if (!valid) return addToast(message, 'error');
        }
        if (formData.bankDetails.ifscCode) {
            const { valid, message } = validateIFSC(formData.bankDetails.ifscCode);
            if (!valid) return addToast(message, 'error');
        }

        // Tax Validation
        if (formData.panNo) {
            const { valid, message } = validatePAN(formData.panNo);
            if (!valid) return addToast(message, 'error');
        }
        if (formData.gstNo) {
            const { valid, message } = validateGST(formData.gstNo);
            if (!valid) return addToast(message, 'error');
        }
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    {formData.smeType === 'Freelancer' && (
                                        <textarea name="address" value={formData.address} onChange={handleInputChange} placeholder="Address *" className="border p-2 rounded text-sm w-full md:col-span-3" rows="2" required />
                                    )}
                                </div>
                            </div>
