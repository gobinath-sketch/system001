...
import { Briefcase, FileText, Grid, ExternalLink, Edit, Check, X } from 'lucide-react';
...
const SalesManagerTeamView = () => {
  ...
  const analyticsData = React.useMemo(() => {
    if (!teamOpportunities.length) return {
      topClients: [],
      typeDist: []
    };
    ...
    const topClients = Object.entries(clientMap).map(([name, value]) => ({
      name,
      value
    ...
    const typeDist = Object.entries(typeMap).map(([name, data]) => ({
      name,
      value: data.value,
      count: data.count
  ...
  return <div className="p-3 sm:p-6 space-y-6 sm:space-y-8 bg-bg-page h-full">
            {/* Header */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div style={glassCardStyle} className="p-6">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <button onClick={() => setShowDocumentModal(true)} className="text-gray-400 hover:text-gray-600">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div style={glassCardStyle} className="p-4 flex flex-col min-h-[300px]">
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <select value={analyticsMember} onChange={e => setAnalyticsMember(e.target.value)} className="text-[10px] border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-0.5 px-1">
                    ...
                    <div className="flex-1 min-h-0">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={analyticsData.typeDist} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                                    {analyticsData.typeDist.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                                </Pie>
                                <Tooltip content={({
                active,
                payload
                ...
                if (active && payload && payload.length) {
                  ...
                  return <div className="bg-white p-2 border border-gray-200 shadow-md rounded text-xs">
                ...
                <div style={glassCardStyle} className="p-4 flex flex-col min-h-[300px]">
                    ...
                    <div className="flex-1 overflow-auto space-y-2">
                        {analyticsData.topClients.length === 0 ? <div className="h-full flex items-center justify-center text-gray-400 text-xs">No data available</div> : analyticsData.topClients.map((client, index) => <div key={index} className="flex items-center justify-between p-2 rounded-lg bg-gray-50">
                                    <div className="flex items-center gap-2">
                ...
                <div style={glassCardStyle} className="p-4 flex flex-col min-h-[300px]">
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <select value={targetPeriod} onChange={e => setTargetPeriod(e.target.value)} className="text-[10px] border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-0.5 px-1">
                ...
                return <tr key={member._id} className="border-b hover:bg-gray-50">
                                            ...
                                            <td className="text-right py-1.5 px-1.5">
                                                {editingTarget === member._id ? <input type="number" value={targetValue} onChange={e => setTargetValue(e.target.value)} className="w-full text-right border border-gray-300 rounded px-1 py-0.5 text-xs focus:ring-1 focus:ring-blue-500" autoFocus onFocus={() => (targetValue == 0 || targetValue == '0') && setTargetValue('')} /> : <span className="font-medium text-xs">{formatCompactCurrency(currentTarget)}</span>}
                                            </td>
                                            <td className="text-center py-1.5 px-1.5">
                                                {editingTarget === member._id ? <div className="flex items-center justify-center gap-1">
                                                        <button onClick={() => handleSaveTarget(member._id)} className="text-green-600 hover:text-green-700">
                                                            <Check size={12} />
                                                        </button>
                                                        <button onClick={() => setEditingTarget(null)} className="text-red-600 hover:text-red-700">
                                                            <X size={12} />
                                                        </button>
                                                    </div> : <button onClick={() => handleEditTarget(member._id, currentTarget, targetPeriod)} className="text-blue-600 hover:text-blue-700">
                                                        <Edit size={12} />
                                                    </button>}
                                            </td>
                                        </tr>;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div style={glassCardStyle} className="p-4 flex flex-col min-h-[300px]">
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] text-gray-500">Filter:</span>
                            <select value={selectedMember} onChange={e => setSelectedMember(e.target.value)} className="text-[10px] border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 py-0.5 px-1">
                                <option value="all">All Team</option>
                                {teamMembers.map(member => <option key={member._id} value={member._id}>{member.name}</option>)}
                            </select>
                        </div>
                ...
                if (active && payload && payload.length) {
                  const data = payload[0].payload;
                  return <div className="bg-white p-2 border border-gray-200 shadow-lg rounded-lg text-xs">
                ...
                fontSize: '10px'
                                ...
                                <Line type="monotone" dataKey="revenue" name="Total Revenue" stroke="#10B981" strokeWidth={2} dot={{
                r: 3
              }} activeDot={{
                r: 5
              }} />
            ...
            {/* Document Modal */}
            {showDocumentModal && <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                        <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[80vh] flex flex-col">
                            ...
                            <div className="overflow-auto p-3 sm:p-6 flex-1">
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody>
                                        {teamOpportunities.map(opp => <tr key={opp._id} className="border-b border-gray-100 hover:bg-gray-50">
