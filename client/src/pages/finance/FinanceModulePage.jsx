...
const FinanceModulePage = () => {
    ...
    const filteredOpportunities = opportunities.filter(op =>
        op.opportunityNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
        op.client?.companyName?.toLowerCase().includes(searchTerm.toLowerCase())
    );
                    ...
                    <table className="min-w-full text-center text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {loading ? (
                                ...
                                <tr><td colSpan="6" className="p-6 text-center text-gray-500">No opportunities found</td></tr>
                            ) : (
                                filteredOpportunities.map(op => (
                                    <tr key={op._id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-center font-medium text-green-600">
                                            {op.financeDetails?.clientReceivables?.amountReceivable
                                                ? `₹ ${op.financeDetails.clientReceivables.amountReceivable.toLocaleString()}`
                                                : '-'}
                                        </td>
                                        <td className="px-6 py-4 text-center font-medium text-red-600">
                                            {op.financeDetails?.vendorPayables?.trainerFinalExpenses
                                                ? `₹ ${op.financeDetails.vendorPayables.trainerFinalExpenses.toLocaleString()}`
                                                : '-'}
                                        </td>
