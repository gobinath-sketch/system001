...
const ProgramExecutionList = () => {
  ...
  const filteredOpportunities = opportunities.filter(opp => opp.opportunityNumber?.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return <div className="p-3 sm:p-5">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 sm:mb-8 gap-3">
                ...
                <div className="relative w-full sm:w-64">
                    <input type="text" placeholder="Search by Opp ID..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:border-brand-blue" />
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full text-left text-sm whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredOpportunities.map(opp => <tr key={opp._id} className="hover:bg-blue-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center space-x-2">
                                            <div className="flex-1 bg-gray-200 rounded-full h-2 w-32">
                                                <div className={`h-full rounded-full transition-all duration-500 ${(opp.progressPercentage || 0) >= 100 ? 'bg-green-500' // Completed
                    : 'bg-brand-blue' // In Progress (Theme Color)
                      ...
                      width: `${opp.progressPercentage || 0}%`
                                    ...
                                    <td className="px-6 py-4">
                                        <Link to={`/opportunities/${opp._id}`} className="text-brand-blue hover:text-brand-gold font-medium">
                                            Manage
                                        </Link>
                                    </td>
                                </tr>)}
