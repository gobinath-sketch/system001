...
import { Users, Briefcase, Grid, Target } from 'lucide-react';
...
const ManagerDashboard = ({
  ...
  const StatCard = ({
    ...
    subtext,
    onClick
  }) => <div onClick={onClick} style={glassCardStyle} className={`p-6 flex items-center space-x-4 ${onClick ? 'cursor-pointer hover:shadow-[0_8px_32px_0_rgba(255,255,255,0.3)] transition-all' : ''}`}>
  ...
  // Chart Theme Colors
  '#1e40af'; // blue (achieved)
  '#FCD34D'; // yellow (target)
  return <div className="p-6 space-y-8 bg-bg-page h-full">
            {/* Header */}
            {/* Header Removed */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Total Clients */}
                <StatCard title="Total Clients" value={stats?.totalClients || 0} icon={Grid} bgColor="bg-blue-100" iconColor="text-blue-600" subtext="Handled by team" onClick={() => navigate('/clients')} />
                ...
                {/* Team Members */}
                <StatCard title="Team Members" value={stats?.teamMembersCount || 0} icon={Users} bgColor="bg-green-100" iconColor="text-green-600" subtext="Sales Executives" />
            </div>
            ...
            <div style={glassCardStyle} className="p-6">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex gap-3">
                        <select value={selectedEmployee} onChange={e => setSelectedEmployee(e.target.value)} className="text-sm border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            <option value="all">All Team Members</option>
                            {teamPerformance.map(member => <option key={member.userId} value={member.userId}>
                                    {member.name}
                                </option>)}
                        </select>
                        <select value={timeline} onChange={e => setTimeline(e.target.value)} className="text-sm border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
            ...
            height: '350px'
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={selectedEmployee === 'all' ? teamPerformance : teamPerformance.filter(m => m.userId === selectedEmployee)} margin={{
                top: 20,
                right: 30,
                left: 20,
                bottom: 60
                    ...
                    <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                        ...
                        <div className="space-y-4">
                            ...
                            <button onClick={() => setShowTargetModal(true)} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-primary-blue text-white rounded-md hover:bg-blue-700 transition-colors">
                                <Target size={18} />
                                Set New Target
                            </button>
            ...
            {/* Set Target Modal */}
            {showTargetModal && <SetTargetModal onClose={() => setShowTargetModal(false)} onSuccess={() => {
